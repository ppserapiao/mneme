...
1. **The SDK core must stay dependency-light.** Pulling `@huggingface/transformers` + `onnxruntime-node` into `@mneme/sdk` would more than double install size (~50 MB) for users who don't need semantic search.
...
**1. Ship a separate `@mneme/embedder-local` package, not a bundled feature.**

`@mneme/sdk` exports the `Embedder` interface and accepts an `embedder` option in `new Mneme()`. `@mneme/embedder-local` lives at `packages/embedder-local`, depends on `@huggingface/transformers`, and implements `Embedder` with a `LocalEmbedder` class. `@mneme/sdk` does NOT depend on `@mneme/embedder-local` — users opt in by installing both.
...
- Users who don't need semantic search pay zero install cost — `@mneme/sdk` stays tiny.
- Local-first promise holds end-to-end — no API keys, no third-party requests for embeddings.
- The `Embedder` interface is implementation-agnostic; future packages (`@mneme/embedder-voyage`, `@mneme/embedder-openai`, custom) drop in without SDK changes.
...
- **Bundle `@huggingface/transformers` directly in `@mneme/sdk`.** Rejected — install bloat for users who don't want embeddings, plus locks every SDK consumer to the ONNX runtime even on environments where it's not viable (edge functions, browser without WASM fallback).
- **Default to a hosted embedder (Voyage / OpenAI).** Rejected — sends memory contents to a third party on every write. Direct contradiction of the local-first promise. Hosted embedders ship as separate packages (`@mneme/embedder-voyage`) for users who explicitly opt in.
